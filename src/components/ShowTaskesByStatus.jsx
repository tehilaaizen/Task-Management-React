    ...
    const itemTemplate = (task) => {
        return (
            <>
                ...
                <div>
                    <div className="flex flex-column xl:flex-row xl:align-items-start gap-4 task-item">
                        ...
                        <div className="flex flex-column lg:flex-row justify-content-between align-items-center xl:align-items-start lg:flex-1 gap-4">
                            <div className="flex flex-column align-items-center lg:align-items-start gap-3">
                                <div className="flex flex-column gap-1 task-content">
                                    <div className="text-2xl font-bold text-900 task-title">{task.title}</div>
                                    <div className="text-700 font-alt task-desc">{task.description}</div>
                            ...
                            <div className="flex flex-row lg:flex-column align-items-center lg:align-items-end gap-4 lg:gap-2">
                                ...
                                <Button icon="pi pi-trash" onClick={() => dispatch(removeTask({ projectId: project.id, taskId: task.id }))} className='my-button' />
                                <Button icon="pi pi-pencil" onClick={() => setShowAdd(true)} />
                                {showAdd && (
                                    <EditTask projectId={project.id} task={task} visible={showAdd} onClose={() => setShowAdd(false)} />
                                )}
                                <Button icon='pi pi-angle-double-right' onClick={() => dispatch(changeStatus({ projectId: project.id, taskId: task.id }))} className='my-button' />
                            </div>
                        </div>
                    </div>
                </div>
